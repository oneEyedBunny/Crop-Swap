  ...
  describe("POST /users", function () {
    ...
    it("Should trim firstName", function () {
      return chai
        .request(app)
        .post("/users")
        .send({ firstName: ` ${firstName} `, lastName, password, username , email, city, zipCode })
        .then(res => {
          expect(res).to.have.status(201);
    ...
    it("Should reject users with duplicate username", function () {
      return User
        .create({ firstName, lastName, username, password, email, city, zipCode })
        .then(() => {
          return chai
            .request(app)
            .post("/users")
            .send({ firstName, lastName, username, password, email, city, zipCode });
        })
